...
require_once __DIR__ . '/../Models/FormModel.php'; // Nueva dependencia
...
class ApiController {
    ...
    /**
     * --- SEGURIDAD: VALIDACION CSRF ---
     * Compara el token enviado en la peticion (header o POST) con el de la sesion.
     */
    private function validarCSRF() {
        $token_recibido = $_SERVER['HTTP_X_CSRF_TOKEN'] ?? $_POST['csrf_token'] ?? null;
        if (!$token_recibido || !isset($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $token_recibido)) {
            // Registrar intento fallido para auditoría persistente
            $ip = $_SERVER['REMOTE_ADDR'] ?? 'unknown';
            $uri = $_SERVER['REQUEST_URI'] ?? 'unknown';
            if (function_exists('log_csrf_attempt')) {
                log_csrf_attempt(['ip' => $ip, 'uri' => $uri, 'payload' => null]);
            } else {
                error_log("CSRF VALIDATION FAILED (no audit function) from {$ip} on {$uri}");
            }
...
public function handleDatosCliente() {
    ...
    public function handleEliminarConsulta() {
        ...
        $this->validarCSRF(); // SEGURIDAD: PROTECCION CSRF
        header('Content-Type: application/json');
    ...
    public function procesarNuevaConsulta() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            // Asegurar que la sesión esté iniciada para validar CSRF y mantener mensajes de formulario
            if (session_status() === PHP_SESSION_NONE) {
                session_start();
            }

            // VALIDACION CSRF: aceptar token por POST o por header X-CSRF-TOKEN
            $token_recibido = $_POST['csrf_token'] ?? $_SERVER['HTTP_X_CSRF_TOKEN'] ?? null;
            if (!$token_recibido || !isset($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $token_recibido)) {
                // Para envíos desde el formulario público redirigimos con mensaje de error
                $_SESSION['form_errors'] = 'ERROR DE SEGURIDAD: TOKEN CSRF INVÁLIDO. POR FAVOR RECARGA LA PÁGINA E INTÉNTALO DE NUEVO.';
    ...
    /**
     * Devuelve un token CSRF actual (o lo regenera) para uso por AJAX.
     * Endpoint: /api/csrf-refresh
     */
    public function csrfRefresh() {
        // No exigir autenticación; la sesión debe existir para devolver token
        if (session_status() === PHP_SESSION_NONE) session_start();

        header('Content-Type: application/json');
        // Regenerar el token por seguridad
