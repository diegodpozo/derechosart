...
class AuthController {
    ...
    public function procesarLogin() {
        ...
        if (!$token_recibido || !isset($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $token_recibido)) {
            if (function_exists('log_csrf_attempt')) { log_csrf_attempt(['ip'=>$_SERVER['REMOTE_ADDR']??'unknown','uri'=>$_SERVER['REQUEST_URI']??'unknown','payload'=>null]); }
            $_SESSION['login_error'] = 'ERROR DE SEGURIDAD: TOKEN CSRF INVALIDO.';
        ...
        if (empty($username) || empty($contrasena)) {
            ...
            try {
                ...
                if ($usuario && password_verify($contrasena, $usuario['password'])) {
                    // --- SEGURIDAD: REGENERAR ID DE SESION TRAS LOGIN EXITOSO ---
    ...
    public function procesarCambiarContrasena() {
        ...
        if (!$token_recibido || !isset($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $token_recibido)) {
            if (function_exists('log_csrf_attempt')) { log_csrf_attempt(['ip'=>$_SERVER['REMOTE_ADDR']??'unknown','uri'=>$_SERVER['REQUEST_URI']??'unknown','payload'=>null]); }
            $_SESSION['errores_cambio'] = ['ERROR DE SEGURIDAD: TOKEN CSRF INVALIDO.'];
