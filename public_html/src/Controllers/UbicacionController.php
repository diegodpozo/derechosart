...
class UbicacionController {
    ...
    public function sincronizarUbicaciones() {
        ...
        // VALIDACION CSRF: aceptar token por header X-CSRF-TOKEN o por POST
        ...
        if (!$token_recibido || !isset($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $token_recibido)) {
            // Log de intento fallido persistente
            $ip = $_SERVER['REMOTE_ADDR'] ?? 'unknown';
            $uri = $_SERVER['REQUEST_URI'] ?? 'sin_uri';
            if (function_exists('log_csrf_attempt')) {
                log_csrf_attempt(['ip' => $ip, 'uri' => $uri, 'payload' => null]);
            } else {
                error_log("CSRF VALIDATION FAILED in UbicacionController from {$ip} on {$uri}");
            }
