...
<script>
    window.gestionMensajeArt = '<?= addslashes($_SESSION['gestion_mensaje_art']) ?>';
</script>
...
<script>
    window.gestionErrorArt = '<?= addslashes($_SESSION['gestion_error_art']) ?>';
</script>
...
<div class="container-fullscreen">
    ...
    <?php foreach ($consultas as $index => $consulta): ?>
        <tr class="<?= $index % 2 === 0 ? 'fila-par' : 'fila-impar' ?>" 
            data-id="<?= $consulta['id'] ?>"
            data-nombre="<?= htmlspecialchars($consulta['nombre']); ?>"
            data-apellido="<?= htmlspecialchars($consulta['apellido']); ?>"
            data-telefono="<?= htmlspecialchars($consulta['telefono']); ?>"
...
<!-- Script JS específico para esta vista -->
<script>
    // Pasamos la URL base a JavaScript para construir URLs directas a archivos
    const BASE_URL = '<?= BASE_URL ?>';
    // Pasamos la URL base de la API a JavaScript (debe pasar por index.php para el enrutamiento)
...
<script src="<?= BASE_URL ?>/js/gestiondb.js?v=2.0"></script>
ipt>
