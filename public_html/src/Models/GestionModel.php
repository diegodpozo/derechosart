...
class GestionModel {
    ...
    // Validar columnas permitidas para ordenar (por seguridad)
    ...
    public function getConsultaById(int $id, ?bool $eliminado_status = false, ?int $user_id = null) {
        ...
        // --- SEGURIDAD: FILTRAR POR USUARIO ASIGNADO SI NO ES ADMIN ---
        if ($user_id !== null) {
            $where_clause .= " AND c.asignado_a = :user_id";
        }
        ...
        try {
            ...
            if ($consulta && isset($consulta['fecha_registro'])) {
                $consulta['fecha_registro'] = convertir_fecha_buenos_aires($consulta['fecha_registro']);
            }
            return $consulta;
        } catch (PDOException $e) {
    ...
    public function guardarNuevaConsulta(array $datos) {
        try {
            ...
            // 1. VALIDACION DE DATOS BASICOS (COMUNES A TODOS)
            ...
            // OBTENER IDS DE CATEGORIAS PARA VALIDACION DINAMICA
            ...
            // 2. VALIDACION DE CAMPOS ESPECIFICOS SEGUN CATEGORIA
            ...
            // 4. INSERCION REGISTRO PRINCIPAL
            ...
            // 5. INSERCION REGISTRO DE DETALLES
            if ($categoria_id == $id_acc) {
