document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target && (e.target.classList.contains('close') || e.target.closest('.close') || e.target.id === 'btnCancelarArt')) {
    ...
    function mostrarFormularioEdicion(c) {
        ...
        let h = `<div style="max-height: 70vh; overflow-y: auto;">
            ...
            <div class="modal-form-group"><label>LOCALIDAD:</label><select id="editLocalidad" name="localidad_id" class="modal-input">
                <option value="${c.localidadId}">${escapeHtml(c.localidad)}</option>
            </select></div>`;

        // CAMPOS ESPECÍFICOS SEGÚN CATEGORÍA
        if (c.categoriaId == ID_ACCIDENTES || c.categoriaId == ID_ENFERMEDADES) {
            h += `<div class="modal-form-group"><label>EDAD:</label><input type="number" name="${c.categoriaId == ID_ACCIDENTES ? 'edad' : 'edad_enf'}" value="${c.edad}" class="modal-input"></div>`;
            h += `<div class="modal-form-group"><label>ART:</label><select name="${c.categoriaId == ID_ACCIDENTES ? 'art_id' : 'art_id_enf'}" class="modal-input">
                <option value="">SIN ART / NO SABE</option>
                ${TODAS_ART_EMPRESAS.map(a => `<option value="${a.id}" ${c.artId == a.id ? 'selected' : ''}>${escapeHtml(a.nombre)}</option>`).join('')}
            </select></div>`;
            h += `<div class="modal-form-group"><label>SUELDO REGISTRADO:</label><input type="text" name="${c.categoriaId == ID_ACCIDENTES ? 'sueldo_registrado' : 'sueldo_registrado_enf'}" value="${c.sueldoRegistrado}" class="modal-input numeric-input"></div>`;
            h += `<div class="modal-form-group"><label>LESIÓN / DOLENCIA:</label><input type="text" name="${c.categoriaId == ID_ACCIDENTES ? 'descripcion_lesion' : 'descripcion_lesion_enf'}" value="${escapeHtml(c.descripcionLesion)}" class="modal-input"></div>`;
            
            if (c.categoriaId == ID_ACCIDENTES) {
                h += `<div class="modal-form-group"><label>FECHA ACCIDENTE:</label><input type="date" name="fecha_accidente_acc" value="${c.fechaAccidente}" class="modal-input"></div>`;
            } else {
                h += `<div class="modal-form-group"><label>ANTIGÜEDAD:</label><input type="text" name="antiguedad_laboral" value="${escapeHtml(c.antiguedadLaboral)}" class="modal-input"></div>`;
            }
        } else if (c.categoriaId == ID_DESPIDOS) {
            h += `<div class="modal-form-group"><label>SUELDO TOTAL:</label><input type="text" name="sueldo_total" value="${c.sueldoTotal}" class="modal-input numeric-input"></div>`;
            h += `<div class="modal-form-group"><label>SITUACIÓN ACTUAL:</label><select name="situacion_actual" class="modal-input">
                <option value="trabajando" ${c.situacionActual == 'trabajando' ? 'selected' : ''}>TRABAJANDO</option>
                <option value="despedido" ${c.situacionActual == 'despedido' ? 'selected' : ''}>DESPEDIDO</option>
                <option value="renuncio" ${c.situacionActual == 'renuncio' ? 'selected' : ''}>RENUNCIÓ</option>
            </select></div>`;
        }
        ...
        if (pSel && lSel) { 
            pSel.addEventListener('change', () => { 
                fetch(`${BASE_API_URL}/localidades?provincia_id=${pSel.value}`)
                    .then(r => r.json())
                    .then(d => { 
                        lSel.innerHTML = d.localidades.map(l => `<option value="${l.id}">${escapeHtml(l.nombre)}</option>`).join(''); 
                    }); 
            }); 
        }
    ...
    if (artList) artList.addEventListener('click', (e) => { 
        ...
        if (btn && confirm('¿ELIMINAR ART?')) {
            e.preventDefault(); // EVITAR CIERRES ACCIDENTALES
            e.stopPropagation();
            fetchWithCsrf(`${BASE_API_URL}/eliminar_art`, { 
                ...
                body: JSON.stringify({ id: btn.dataset.artId })
            })
            .then(r => r.json())
            .then(res => {
                if (res.success) {
                    ...
                    artMsg.innerHTML = '<strong>' + res.message + '</strong>'; // MEJOR VISIBILIDAD
                    artMsg.className = 'message-area error'; 
